...
public class NewChampion {
    ...
    public List<ChampionData> fetchChampionData() throws IOException {
        ...
        if (responseCode == HttpURLConnection.HTTP_OK) {
            try (BufferedReader in = new BufferedReader(new InputStreamReader(request.getInputStream()))) {
                ...
                if (championInfo.isEmpty()) {
                    ...
                    for (int i = 0; i < championInfo.length(); i++) {
                        JSONObject playerData = championInfo.getJSONObject(i);

                        String championName = playerData.getString("championName");
                        int championID = playerData.getInt("championId");
                        int magicDamage = playerData.getInt("magicDamageDealt");
                        int physicalDamage = playerData.getInt("physicalDamageDealt");
                        int totalDamage = playerData.getInt("totalDamageDealt");
                        int trueDamage = playerData.getInt("trueDamageDealt");
    ...
    public List<ChampionData> getTopThreeChampions() throws IOException {
        List<ChampionData> allChampions = fetchChampionData();
        allChampions.sort(Comparator.comparingInt(ChampionData::getMasteryPoints).reversed());
        return allChampions.size() > 3 ? allChampions.subList(0, 3) : allChampions;
    }
    ...
    static class ChampionData {
        ...
        public ChampionData(String championName, int championID, int magicDamage, int physicalDamage, int totalDamage,
                            int trueDamage, int kills, int masteryPoints) {
